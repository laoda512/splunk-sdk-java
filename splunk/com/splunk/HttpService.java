...
public class HttpService {
	 ...
	 *            The port number of the service.
	 * @param scheme
	 *            Scheme for accessing the service ({@code http} or
	 *            {@code https}).
	 */
	...
	/**
	 * Constructs a new {@code HttpService} instance using the given host, port,
	 * and scheme, and instructing it to use the specified HTTPS handler.
	 * 
	 * @param host
	 *            The host name of the service.
	 * @param port
	 ...
	 * @param scheme
	 *            scheme Scheme for accessing the service ({@code http} or
	 * @param path
	 *            path of the service
	 * @param httpsHandler
	 */
	public HttpService(String host, int port, String scheme,
			String servicePath, URLStreamHandler httpsHandler) {
		this.host = host;
		this.port = port;
		this.scheme = scheme;
	...
	public String getPrefix() {
		if (this.prefix == null)
			this.prefix = String.format("%s://%s:%s", this.scheme, this.host,
					this.port);
		return this.prefix;
	}
	...
	/**
	 * Issues a POST request against the service using a given path and form
	 * arguments.
	 * 
	 * @param path
	 *            The request path.
	...
	public ResponseMessage post(String path, Map<String, Object> args) {
		...
		request.getHeader().put("Content-Type",
	...
	/**
	 * Issues a DELETE request against the service using a given path and query
	 * arguments.
	 * 
	 * @param path
	 *            The request path.
	...
	public static Connecter defaultConnecter;
	...
	public Connecter getConnecter() {
		if (connecter == null) {
			if (defaultConnecter != null)
				connecter = defaultConnecter;
			else
				connecter = new Connecter() {
					 ...
					 * @return The HTTP response.
					 */
					public ResponseMessage send(HttpService httpService,
							String path, RequestMessage request) {
						// Construct a full URL to the resource
						// if(isJson&&!path.contains("/services/auth/login")) {
						...
						if (cn instanceof HttpsURLConnection) {
							...
							if (null != customSSLSocketFactory)
								httpsURLConnection
										.setSSLSocketFactory(customSSLSocketFactory);
							else
								httpsURLConnection
							...
							if (null != customHostnameVerifier)
								httpsURLConnection
										.setHostnameVerifier(customHostnameVerifier);
							else
								httpsURLConnection
						...
						for (Entry<String, String> entry : header.entrySet())
							cn.setRequestProperty(entry.getKey(),
									entry.getValue());

						// Add default headers that were absent from the request
						// message
						for (Entry<String, String> entry : defaultHeader
								.entrySet()) {
						...
						// Add custom headers
						if (null != HttpService.this.customHeaders) {
							for (Entry<String, String> entry : HttpService.this.customHeaders
									.entrySet()) {
								String key = entry.getKey();
								if (header.containsKey(key))
									continue;
						...
						try {
							input = status >= 400 ? cn.getErrorStream() : cn
									.getInputStream();
						} catch (IOException e) {
							assert (false);
						}

						ResponseMessage response = new ResponseMessage(status,
								input);
						...
						if (status >= 400)
							throw HttpException.create(response,httpService);
	...
	/**
	 * Issue an HTTP request against the service using a given path and request
	 * message.
	 * 
	 * @param path
	 *            The request path.
	...
	public ResponseMessage send(String path, RequestMessage request) {
		//auto add output_mode if overall mode is set. 
		if (getOverallOutputMode() != null) {
			if (!path.contains("output_mode")) {
				if (path.contains("?")) {
					path += "&output_mode=" + getOverallOutputMode();
				} else {
					path += "?output_mode=" + getOverallOutputMode();
				}
			}
		}//TODO: ...
		return getConnecter().send(this, path, request);
	}
	...
	private static SSLSocketFactory createSSLFactory() {
		...
		try {
			...
			throw new RuntimeException("Error setting up SSL socket factory: "
					+ e, e);
		}
	}
	...
	private static OutputMode output_mode = OutputMode.XML;

	/**
	 * Set the overall output mode of response. Pass null to set to default mode, XML.
	 * 
	 * @param outputMode The overall {@code OutputMode}.
	 * 
	 * */
	public static void setOverallOutputMode(OutputMode outputMode) {
		output_mode = outputMode;
	}

	public static OutputMode getOverallOutputMode() {
		return output_mode;
	}

	public boolean isJsonOutputMode() {
		return getOverallOutputMode() == OutputMode.JSON;
	}
